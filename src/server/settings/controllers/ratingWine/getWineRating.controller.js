const { User, Wine, Wine_rating } = require("../../../../database/model/relationships");
...
const getWineRating = async (req, res) => {
    //user id
    const { id } = req.query;
    //product id
    ...
    try {
        ...
        const findProduct = await Wine.findByPk(productId);
        if(!findUser) return res.status(404).json({status: 404, error: "The user does not exist!"});
        if(!findProduct) return res.status(404).json({status: 404, error: "The wine does not exist!"});
        //Search for the rating
        const findRating = await Wine_rating.findOne({ where: { user_id: id, wine_id: productId } });
