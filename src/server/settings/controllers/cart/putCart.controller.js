...
const putCart = async (req, res) => {
    ...
    try {
        ...
        if(!findCart) return res.status(400).json({ status: 404, error: "The cart doesn't exist!"});
        ...
        res.status(200).json({ status: 200, message: "The cart has been created!", data: findCart});
    } catch (error) {
